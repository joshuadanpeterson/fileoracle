...
def answer_query_from_files(
    ...
    the function will use OpenAI's Responses API (hosted file search tool) instead.

    :param query: The original query.
    :param file_paths: List of file paths to process.
    :param use_responses_api: Boolean flag to use the Responses API.
    :param vector_store_id: The hosted vector store ID (read from .env if not provided).
    :return: The answer generated by the LLM with citations, or an API response object.
    """
    if use_responses_api:
        if not vector_store_id:
            vector_store_id = os.getenv("VECTOR_STORE_ID")
        print(f"Using OpenAI's Responses API with vector store ID: {vector_store_id}")
        ...
        if not answer or answer.strip() == "":
            answer = (
                "The documents did not provide enough context to generate an answer."
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        answer = answer_query_from_files(query, results, use_responses_api=True)
        print("\nFinal Answer:")
        print(answer)
    else:
